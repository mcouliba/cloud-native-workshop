...
.>_ GitOps Export
----
--- Export Kubernetes resources for GitOps from my-project{{USER_ID}} to staging-project{{USER_ID}} ---
...
git remote add origin http://gogs-gogs-server.workshop-infra.svc:3000/user{{USER_ID}}/gitops-staging-project.git
----
...
Once done, in {{ GOGS_URL }}/user{{USER_ID}}/gitops-staging-project, `*refresh the page of your 'gitops-staging-project' repository*`. You should 
...
|Repository URL
|http://gogs-gogs-server.workshop-infra.svc:3000/user{{USER_ID}}/gitops-staging-project.git
...
and there is no image available on your **staging-project{{USER_ID}}** project. **You are going to fix it in the next lab.**
