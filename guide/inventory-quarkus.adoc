...
`*run 'Task: Run Task...' ->  'Inventory - Build'*`
...
`*right-click on your '{{PROJECT}}' project and select 'New Component'*`
...
Now, `*let’s create a route for the component*` by right-clicking on the **inventory component**, 
...
Finally, `*push the component to the OpenShift cluster*` by right-clicking on the **inventory component**, 
...
Applying URL changes
 ✓  URL inventory: http://inventory-coolstore-{{PROJECT}}.{{APPS_HOSTNAME_SUFFIX}} created

Pushing to component inventory of type binary
 ✓  Checking files for pushing [4ms]
 ✓  Waiting for component to start [42s]
 ✓  Syncing files to the component [18s]
