...
----
oc patch dc/inventory-coolstore --patch '{"spec": {"template": {"metadata": {"annotations": {"sidecar.istio.io/inject": "true"}}}}}' -n staging-project{{USER_ID}} # <1>
oc patch dc/inventory-coolstore --patch '{"spec": {"template": {"spec": {"containers": [{"name": "inventory-coolstore", "command" : ["/bin/bash"], "args": ["-c", "until $(curl -o /dev/null -s -I -f http://127.0.0.1:15000); do echo \"Waiting for Istio Sidecar...\"; sleep 1; done; sleep 10; /usr/local/s2i/run"]}]}}}}' -n staging-project{{USER_ID}} # <2>
oc rollout latest dc/inventory-coolstore -n staging-project{{USER_ID}} # <3>
----
...
----
$ oc get pods -ldeploymentconfig=inventory-coolstore -n staging-project{{USER_ID}}
...
.>_ workshop-tools terminal
----
$ oc get pods -ldeploymentconfig=catalog-coolstore -n staging-project{{USER_ID}}
NAME                          READY     STATUS    RESTARTS   AGE
catalog-coolstore-4-xnwfp   2/2       Running   0          39s
$ oc get pods -ldeploymentconfig=gateway-coolstore -n staging-project{{USER_ID}}
NAME                        READY     STATUS    RESTARTS   AGE
...
.>_ workshop-tools terminal
----
oc create -n staging-project{{USER_ID}} -f /projects/workshop/labs/gateway-vertx/openshift/istio-gateway.yml
sed s/PROJECT/staging-project{{USER_ID}}/g /projects/workshop/labs/gateway-vertx/openshift/virtualservice.yml | oc create -n staging-project{{USER_ID}} -f -
----

To confirm that the **Istio Gateway** is properly configured, 
`*click on http://istio-ingressgateway-istio-system.{{APPS_HOSTNAME_SUFFIX}}/staging-project{{USER_ID}}/api/products[http://istio-ingressgateway-istio-system.{{APPS_HOSTNAME_SUFFIX}}/staging-project{{USER_ID}}/api/products^]*`
...
.>_ workshop-tools terminal
----
oc set env dc/web-coolstore -n staging-project{{USER_ID}} COOLSTORE_GW_ENDPOINT=http://istio-ingressgateway-istio-system.{{APPS_HOSTNAME_SUFFIX}}/staging-project{{USER_ID}}
oc rollout latest dc/web-coolstore -n staging-project{{USER_ID}}
----
...
image:{% image_path kiali-show-traces.png %}[Kiali- Show Traces, 700]

You should be redirected to the **Distributed Tracing menu**.
