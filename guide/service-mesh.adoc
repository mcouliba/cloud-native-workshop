...
----
oc patch dc/inventory-coolstore --patch '{"spec": {"template": {"metadata": {"annotations": {"sidecar.istio.io/inject": "true"}}}}}' -n {{PROJECT}} # <1>
oc patch dc/inventory-coolstore --patch '{"spec": {"template": {"spec": {"containers": [{"name": "inventory-coolstore", "command" : ["/bin/bash"], "args": ["-c", "until $(curl -o /dev/null -s -I -f http://127.0.0.1:15000); do echo \"Waiting for Istio Sidecar...\"; sleep 1; done; sleep 10; /usr/local/s2i/run"]}]}}}}' -n {{PROJECT}} # <2>
oc rollout latest dc/inventory-coolstore -n {{PROJECT}} # <3>
----
...
----
$ oc get pods -ldeploymentconfig=inventory-coolstore -n {{PROJECT}}
...
.>_ workshop-tools terminal
----
$ oc get pods -ldeploymentconfig=catalog-coolstore -n {{PROJECT}}
NAME                          READY     STATUS    RESTARTS   AGE
catalog-coolstore-4-xnwfp   2/2       Running   0          39s
$ oc get pods -ldeploymentconfig=gateway-coolstore -n {{PROJECT}}
NAME                        READY     STATUS    RESTARTS   AGE
...
.>_ workshop-tools terminal
----
oc create -n {{PROJECT}} -f /projects/workshop/labs/gateway-vertx/openshift/istio-gateway.yml
sed s/PROJECT/{{PROJECT}}/g /projects/workshop/labs/gateway-vertx/openshift/virtualservice.yml | oc create -n {{PROJECT}} -f -
----
...
.>_ workshop-tools terminal
----
oc set env dc/web-coolstore -n {{PROJECT}} COOLSTORE_GW_ENDPOINT=http://istio-ingressgateway-istio-system.{{APPS_HOSTNAME_SUFFIX}}/{{PROJECT}}
oc rollout latest dc/web-coolstore -n {{PROJECT}}
----
