...
----
$ oc annotate route/web-coolstore haproxy.router.openshift.io/balance=roundrobin
----
====
...
.>_ workshop-tools terminal
----
$ oc set resources dc/web-coolstore --limits=cpu=400m,memory=512Mi --requests=cpu=200m,memory=256Mi
----
...
.>_ workshop-tools terminal
----
$ oc autoscale dc/web-coolstore --min 1 --max 5 --cpu-percent=40
----
...
.>_ workshop-tools terminal
----
$ httperf --server web-coolstore.{{PROJECT}}.svc --port 8080 --num-conn 100 --num-calls 800 --rate 10 --timeout 5 --verbose
----
...
.>_ workshop-tools terminal
----
$ oc get pods -l deploymentconfig=catalog-coolstore #<1>
...
.>_ workshop-tools terminal
----
$ oc delete pods -l deploymentconfig=catalog-coolstore
----
