...
git remote add origin {{ GOGS_URL }}/{{OPENSHIFT_USER}}/gitops-cn-project.git
----
...
But only the **Web Service** is up and running. Indeed, this is the only Service you deployed using s2i feature.
For the others, https://marketplace.visualstudio.com/items?itemName=redhat.vscode-openshift-connector[OpenShift Connector^]
does not create Image or BuildConfig. Using odo underneath, it directly injects the binary into the running Runtime Container.
